    ...
    public class ScrollableControl : Control
    {
        ...
        protected virtual void OnScroll(ScrollEventArgs se)
        {
            ...
            for (int i = 0; i < Controls.Count; i++)
            {
                ...
                if (!init)
                {
                    _scrollMinX = Controls[i].uwfOffset.X + Controls[i].Location.X;
                    _scrollMaxX = Controls[i].uwfOffset.X + Controls[i].Location.X + Controls[i].Width;
                    _scrollMinY = Controls[i].uwfOffset.Y + Controls[i].Location.Y;
                    _scrollMaxY = Controls[i].uwfOffset.Y + Controls[i].Location.Y + Controls[i].Height;
                    init = true;
                }

                if (Controls[i].uwfOffset.X + Controls[i].Location.X < _scrollMinX)
                    _scrollMinX = Controls[i].uwfOffset.X + Controls[i].Location.X;
                if (Controls[i].uwfOffset.Y + Controls[i].Location.Y < _scrollMinY)
                    _scrollMinY = Controls[i].uwfOffset.Y + Controls[i].Location.Y;
                if (Controls[i].uwfOffset.X + Controls[i].Location.X + Controls[i].Width > _scrollMaxX)
                    _scrollMaxX = Controls[i].uwfOffset.X + Controls[i].Location.X + Controls[i].Width;
                if (Controls[i].uwfOffset.Y + Controls[i].Location.Y + Controls[i].Height > _scrollMaxY)
                    _scrollMaxY = Controls[i].uwfOffset.Y + Controls[i].Location.Y + Controls[i].Height;
            }
